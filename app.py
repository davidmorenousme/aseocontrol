...
# ── Rutas de prueba (eliminar en producción) ──
@app.route('/test/ausencias')
def test_ausencias():
    verificar_ausencias()
    return 'Verificación de ausencias ejecutada — revisa consola y notificaciones'
 
@app.route('/test/almuerzo')
def test_almuerzo():
    verificar_almuerzo()
    return 'Verificación de almuerzo ejecutada — revisa consola y notificaciones'
 
 
# ── Scheduler: dos jobs independientes cada minuto ──
def job_ausencias():
